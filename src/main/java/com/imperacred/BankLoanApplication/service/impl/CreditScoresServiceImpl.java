...
<<<<<<< HEAD
    ...
    private String getRiskCategory(int score) {
        logger.debug("Evaluating risk category for score: {}", score);
        if (score >= 750) {
            logger.debug("Risk category: Low");
            return "Low";
        } else if (score >= 600) {
            logger.debug("Risk category: Medium");
            return "Medium";
        } else {
            logger.debug("Risk category: High");
            return "High";
        }
    }
    ...
    public CreditScoresDTO getCreditScoreById(int id) {
        ...
        // Retrieve the credit score from the database
        CreditScores creditScore = creditScoresRepository.findById(id).orElse(null);
