...
const DocumentUploadPage = () => {
  ...
  const checkAssignmentStatus = async (docs) => {
    ...
    if (types.includes('salary slip') && types.includes('bank statement')) {
      ...
      try {
        ...
        // If lead is assigned and any document has reuploadRequested true, redirect to LeadStatusPage
  ...
  const hasUploadedAllRequiredDocs = () => {
    ...
    return types.includes('salary slip') && types.includes('bank statement');
  };
    ...
    <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded shadow">
      ...
      <form onSubmit={handleUpload} className="space-y-4">
        ...
        <select
          ...
          className="w-full p-2 border rounded"
          required
          disabled={hasUploadedAllRequiredDocs()}
        >
          <option value="">Select Document Type</option>
          {!documents.some(doc => doc.documentType.toLowerCase() === 'salary slip') && (
      ...
      {hasUploadedAllRequiredDocs() && (
        <p className="mt-2 text-green-600 text-sm font-medium text-center">
          Both required documents uploaded. Upload disabled.
        </p>
      )}
